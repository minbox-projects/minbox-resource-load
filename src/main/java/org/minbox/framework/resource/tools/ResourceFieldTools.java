...
public class ResourceFieldTools {
    ...
    public static Field getField(Class objectClass, String fieldName) {
        // cache from memory
        Field sourceField = ResourceContext.getPushFieldFromCache(objectClass.getName(), fieldName);
        ...
        if (ObjectUtils.isEmpty(sourceField)) {
            ...
            // cache to memory
            ResourceContext.setPushFieldToCache(objectClass.getName(), fieldName, sourceField);
        }
        return sourceField;
    }
    ...
    public static List<ResourceField> getResourceFields(Method method) {
        // get from cache
        List<ResourceField> resourceFields = ResourceContext.getResourceFieldFromCache(method);

        // loading from method declared
        if (ObjectUtils.isEmpty(resourceFields)) {
            resourceFields = loadMethodResourceFields(method);
            // set to cache
            ResourceContext.setResourceFieldToCache(method, resourceFields);
        }
        return resourceFields;
    }
}
